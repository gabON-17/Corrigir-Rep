...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (localStorage.getItem("id_atual") == null) {
    localStorage.setItem("id_atual", "0");
  }
});
...
function cadastrarPet(e) {
  ...
  let idAtual = Number(localStorage.getItem("id_atual"));
  ...
  const animal = {
    ...
    especie: especie,
    sex: sexo,
    servicesList: listaServicos,
    date: data,
  };

  // SALVAR NO LOCALSTORAGE
  localStorage.setItem(`animal_${idAtual}`, JSON.stringify(animal));

  idAtual++;
  localStorage.setItem("id_atual", idAtual);
